...
class AreaSelection extends StatefulWidget {
  ...
  final Function onSelect; // 选中的回调
  final int initProviceIndex; //省索引
  final int initCityIndex; //城市索引
  final int initCountyIndex;//区索引
  final int initTownIndex; //镇

  AreaSelection({Key key, @required this.onSelect, this.initProviceIndex, this.initCityIndex, this.initCountyIndex, this.initTownIndex}) : super ( key : key);
...
//        }
  ...
  //定义镇控制器
  FixedExtentScrollController townController;
  ...
  void initState() {
    ...
    townController = new FixedExtentScrollController(initialItem: widget.initTownIndex ?? 0);
    tempAreaInfo = {
      'proviceId': provices[selectedProvice].id,
      'cityId': citys[selectedCity].id,
      'countyId': county[selectedCounty].id,
      'address': provices[selectedProvice].name + citys[selectedCity].name + county[selectedCounty].name + town[selectedTown].name,
            ...
            child: Row(
              children: <Widget>[
                Expanded(
                  ...
                  child: CupertinoPicker(
                    ...
                    onSelectedItemChanged: (position){
                      print(position);
                      setState(() {
                        selectedProvice = position;//选的是哪个省
                        citys = provices[selectedProvice].childs;//对应那个省下面的那个城市
                        if(citys.length == 0) {
                          provices[selectedProvice].childs.add(
                            AreaModel.fromMap(
                              {
                              "id": 11010,
                              "name": null,
                        ...
                        selectedCity = 0;//城市先置0
                        county = citys[selectedCity].childs;//区选的是选中的按个城市的下面的第一个
                        if(county.length == 0) {
                          citys[selectedCity].childs.add(
                            AreaModel.fromMap(
                              {
                              "id": 11010,
                              "name": null,
                        ...
                        selectedCounty = 0;//区置0
                        town = county[selectedCounty].childs;
                        //四级菜单为空时
                        if(town.length == 0) {
                          county[selectedCounty].childs.add(
                            AreaModel.fromMap(
                              {
                              "id": 11010,
                              "name": null,
                ...
                Expanded(
                  ...
                  child: CupertinoPicker(
                    backgroundColor: Colors.white,
                    scrollController: townController,
                    itemExtent: 48.0,
                    onSelectedItemChanged: (position){
                      selectedTown = position;
                      passParams();
                    },
                    children: createEachItem(town),
                  ),
                ),
              ],
            ),
